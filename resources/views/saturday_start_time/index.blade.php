...
@section('content')
    <div class="card">
        <table class="table table-striped">
            ...
            <tbody>
                @foreach ($start_times as $e)
                    <tr>
                        <td>{{ $e->id }}</td>
                        <td> {{ date('h:i A', strtotime($e->time)) }}</td>
                        <td>
                            <a href="{{ route('saturday_start_time.show', $e->id) }}" class="pe-lg-2">
                                <i class="fa-solid fa-eye"></i>
                            </a>
                            <a href="{{ route('saturday_start_time.edit', $e->id) }}">
                                <i class="fa-solid fa-pen-to-square" style="color: #004fd6;"></i>
                            </a>
                            <form action="{{ route('saturday_start_time.destroy', $e->id) }}" method="POST"
